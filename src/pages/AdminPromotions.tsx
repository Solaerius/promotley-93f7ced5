...
const AdminPromotions = () => {
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      ...
      toast({ title: "✓ Skapad", description: `Promotionslänk ${form.code} skapad` });
      setForm({ code: "", credits_amount: 10, max_uses: "", expires_at: "" });
  ...
  const toggleActive = async (id: string, active: boolean) => {
    await supabase.from("promotion_links").update({ is_active: !active }).eq("id", id);
    loadPromotions();
  };

  const deletePromotion = async (id: string) => {
    await supabase.from("promotion_links").delete().eq("id", id);
    loadPromotions();
    toast({ title: "Borttagen" });
  };

  const copyLink = (code: string) => {
    navigator.clipboard.writeText(`${window.location.origin}/promo/${code}`);
    toast({ title: "Kopierad!", description: "Länken har kopierats" });
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">Promotioner</h1>
            <p className="text-sm text-muted-foreground">Skapa och hantera promotionslänkar</p>
          </div>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="w-4 h-4 mr-2" />Ny promotion</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Ny promotionslänk</DialogTitle></DialogHeader>
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label>Kod (unik)</Label>
                  <Input value={form.code} onChange={e => setForm({...form, code: e.target.value})} placeholder="VINTER2025" />
                </div>
                <div className="space-y-2">
                  <Label>Antal krediter</Label>
                  <Input type="number" value={form.credits_amount} onChange={e => setForm({...form, credits_amount: parseInt(e.target.value) || 0})} />
                </div>
                <div className="space-y-2">
                  <Label>Max användningar (lämna tomt = obegränsat)</Label>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">Inga promotioner ännu</TableCell></TableRow>
                ) : promotions.map(p => (
                  <TableRow key={p.id}>
