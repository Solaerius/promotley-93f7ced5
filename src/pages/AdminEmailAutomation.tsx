...
const EMAIL_TYPE_LABELS: Record<string, string> = {
  inactive_reminder: "Påminnelse vid inaktivitet",
  reengagement: "Återengagemang",
};
...
export default function AdminEmailAutomation() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error("Error loading email automation data:", err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6 max-w-4xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold">E-postautomation</h1>
            <p className="text-sm text-muted-foreground">Konfigurera automatiska e-postutskick</p>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-lg">
                      {EMAIL_TYPE_LABELS[setting.email_type] || setting.email_type}
                    </CardTitle>
                    <CardDescription>
                      Skickas till användare efter {setting.delay_days} dagars inaktivitet
                    </CardDescription>
                  </div>
                  <Switch
              ...
              <CardContent>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">Fördröjning (dagar):</span>
                  </div>
                  <Input
                    type="number"
                    min={1}
                    max={90}
                    value={setting.delay_days}
                    onChange={(e) => {
                      const val = parseInt(e.target.value);
                      if (val > 0 && val <= 90) updateSetting(setting.id, { delay_days: val });
                    }}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>De senaste 50 automatiskt skickade e-postmeddelandena</CardDescription>
          </CardHeader>
          <CardContent>
            <ScrollArea className="h-[300px]">
              {logs.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-8">Inga utskick ännu</p>
              ) : (
